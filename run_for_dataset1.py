...
# Define a constant for the model cache location
MODEL_CACHE_DIR = "model_cache"
MODEL_CACHE_FILENAME = "mobilenetv2_notop.keras" # Use .keras format for Keras 3 compatibility
...
def fitness_function(X, y, element, alpha=0.8, golden_ratio=1.618, random_seed=42):
    ...
    # 🟢 Convert continuous predictions to discrete binary (0 or 1)
...
# -------------------
# Image Feature Extraction (WITH CACHING)
# -------------------
def extract_image_features(df):
    """
    Extract MobileNetV2 features from a dataframe with 'image_path' and 'label'.
    Caches the MobileNetV2 base model to prevent re-downloading weights.
    """
    
    # 1. Initialize Model with Caching
    base_model = None
    cache_path = os.path.join(os.getcwd(), MODEL_CACHE_DIR, MODEL_CACHE_FILENAME)

    if os.path.exists(cache_path):
        print(f"Loading MobileNetV2 from cache: {cache_path}")
        try:
            # We use compile=False because we only need the feature extractor
            base_model = load_model(cache_path, compile=False) 
        except Exception as e:
            print(f"Error loading cached model: {e}. Re-downloading...")
            base_model = None # Force re-download if load fails
    ...
    if base_model is None:
        ...
        # Save the model
        base_model.save(cache_path)
        print(f"MobileNetV2 cached successfully at: {cache_path}")
    ...
    # The base_model variable is guaranteed to be loaded from cache or downloaded now.
...
def run_qigpso_on_images(df, popsize=20, alpha=0.8, max_iter=100, g0=35, test_size=0.2, random_seed=42, verbose=False):
    ...
    """
    # NOTE: The extract_image_features function no longer accepts base_model argument
    X_features, y_labels = extract_image_features(df) 
    
    # 🛑 Convert string labels to numerical (0 and 1) for TensorFlow/Keras

    le = LabelEncoder()
    y = le.fit_transform(y_labels) # Converts 'PVNS' and 'SNN' into 0s and 1s
